...
public sealed class YamlResourceProviderTests : IDisposable
{
    ...
    {
        await File.WriteAllTextAsync(Path.Join(_tempDir, "en.yaml"), "greeting: Hello");
    ...
    {
        var yaml = "nav:\n  home: Home Page\n  about: About";
        await File.WriteAllTextAsync(Path.Join(_tempDir, "en.yaml"), yaml);
    ...
    {
        await File.WriteAllTextAsync(Path.Join(_tempDir, "fr.yml"), "greeting: Bonjour");
    ...
    public async Task ReloadAsync_ClearsCache_SubsequentReadLoadsFromDisk()
    {
        ...
        await File.WriteAllTextAsync(Path.Join(_tempDir, "en.yaml"), "key: original");
        var first = await sut.GetStringAsync("key", "en");
        first.Should().Be("original");

        await File.WriteAllTextAsync(Path.Join(_tempDir, "en.yaml"), "key: updated");
